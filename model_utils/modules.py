...
class FrequencyAttentionModule(Layer):
    def __init__(self, **kwargs):
        super(FrequencyAttentionModule, self).__init__(**kwargs)
        # self.shared_cnn = Sequential([
        #     Conv2D(1, kernel_size=(7, 1),
        #            strides=1,
        #            padding='same'),
        #     BatchNormalization(),
        #     ReLU()
    ...
    def call(self, inputs, *args, **kwargs):
        ...
        f = Lambda(
            lambda X: tf.nn.avg_pool(x,
                                     ksize=(1, 1, 1, channel_dim),
                                     strides=(1, 1, 1, channel_dim),
                                     padding='VALID'))
        f_max_layer = Lambda(
            lambda X: tf.nn.max_pool(X,
                                     ksize=(1, 1, time_dim, 1),
                                     strides=(1, 1, time_dim, 1),
                                     padding='VALID'))
        ...
        shared_f_max = f_max
        for layer in self.shared_cnn:
            shared_f_max = layer(shared_f_max)

        shared_f_avg = f_avg
        for layer in self.shared_cnn:
            shared_f_avg = layer(shared_f_avg)

        M_f = Add()([shared_f_max, shared_f_avg])
        M_f = Activation(activation='sigmoid')(M_f)

        return M_f


class TimeAttentionModule(Layer):
    def __init__(self, **kwargs):
        super(TimeAttentionModule, self).__init__(**kwargs)
        # self.shared_cnn = Sequential([
        #     Conv2D(1, kernel_size=(1, 7),
        #            strides=1,
        #            padding='same'),
        #     BatchNormalization(),
        #     ReLU()
    ...
    def call(self, inputs, *args, **kwargs):
        ...
        f_max_layer = Lambda(
            lambda X: tf.nn.max_pool(X,
                                     ksize=(1, frequency_dim, 1, 1),
                                     strides=(1, frequency_dim, 1, 1),
                                     padding='VALID'))
...
class TimeFrequencyAttentionModule(Layer):
    def __init__(self, **kwargs):
        super(TimeFrequencyAttentionModule, self).__init__(**kwargs)
        self.frequency_module = FrequencyAttentionModule()
        self.time_module = TimeAttentionModule()

    def call(self, inputs, *args, **kwargs):
        x = inputs
        f_out = self.frequency_module(x)
        mul_freq = f_out * inputs

        t_out = self.time_module(mul_freq)
        mul_time = t_out * mul_freq
        return mul_time
...
class DeformableConvLayer(Conv2D):
    ...
    def build(self, input_shape):
        ...
        print("Kernel Shape", kernel_shape, self.filters * input_dim, input_dim)
    ...
    def call(self, inputs, training=None, **kwargs):
        ...
        print("Offset Shaep", offset.shape)
...
if __name__ == "__main__":
    ...
    x = FrequencyAttentionModule()(x)

    y = tf.random.normal((1, 64, 64, 2))
    y = TimeAttentionModule()(y)

    z = tf.random.normal((1, 64, 64, 2))
    z = TimeFrequencyAttentionModule()(z)
    print(z.shape)
